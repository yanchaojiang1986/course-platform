...
### 新增
- 模块前置筛选机制：进入基础教程模块（00/01/02）前新增评估弹窗，要求先完成学历、专业、工作经验三项筛选。
- 筛选结果持久化：新增 `cp_entry_screening_v1` 本地记录，通过后可直接进入基础教程模块。
- `todolist.md` 新增“页面安全防护（防复制与内容滥用）”待办项，补充前端限制、水印与接口侧防护规划。

### 变更
- 基础教程入口改为“筛选通过后放行”：不通过时拦截进入并提示“暂不满足学习条件”。
- 评分规则落地：支持“全日制大专及以上”为硬门槛，并叠加学历/专业/经验加减分计算。
...
### 变更
- 全项目术语统一：界面与文案统一使用“教程”表述，覆盖前端展示文本、后端提示文案、数据描述与项目文档。
- 模块 00 内容文件重命名为 `00_教程介绍与学习说明.md`，并同步更新模块映射与读取链路。
- 移动端模块页导航新增“自动收缩”机制：进入页面后默认展示，数秒后自动折叠到右下角悬浮按钮，点击可再次展开。
- 导航自动收缩逻辑覆盖总览页与模块页，统一移动端导航交互行为。

### 修复
- 修复术语替换后潜在的内容映射不一致问题，确保模块 00 正常加载新文件名。
- 修复移动端紧凑导航长期遮挡正文的体验问题，同时保留可快速唤起入口。
...
### 变更
- 移动端导航在教程功能页启用“紧凑悬浮模式”：由全宽底部栏切换为右下角竖排悬浮入口，避免长期占据正文底边。
- 仅在总览页保持全宽底部导航，在模块学习页（非 `map`）自动切换为紧凑模式，兼顾入口可达性与阅读空间。
- 抽屉打开时继续隐藏底部入口并锁定页面滚动，避免层叠遮挡。

### 修复
- 修复移动端教程页浏览时底部导航遮挡正文的问题，降低内容被覆盖风险。
- 同步收窄移动端主内容底部占位，避免“已不遮挡但仍预留过大空白”。
...
### 变更
- 教程正文布局改为更强自适应：移除 `ContentViewer` 与知识关卡区域的桌面端窄列限制（`max-w-3xl/5xl`），内容区随窗口宽度扩展，减少左右留白。
...
### 修复
- 修复本地调试时的 `Failed to fetch` 链路问题：恢复并固定前后端运行链路（`9527 -> 4321`），确保教程内容接口可用。
...
### 修复
- 优化触控端（手机/平板）教程阅读体验：正文从“卡片内滚动”切换为“整页流式滚动”。
...
### 新增
- 教程扩展到 16 个模块：新增模块 12/13/14/15 及对应内容文件、随堂检测、关卡题、互动数据、实战场景。
...
### 变更
- 教程地图与模块卡按会员分层展示全部模块（含锁定态），并统一权限提示流程。
...
### 变更
- 清除教程内容中的 AI 测试教学内容：删除模块 07 的“AI 辅助测试”章节。
- 模块 11 的 AI 相关练习题与章节检测题替换为数据库安全操作、UAT 术语等基础测试知识。
- 教程说明、学习路线与 AI 教辅系统提示中移除“AI 辅助测试”教程覆盖描述。
...
- 去除教程标题与侧栏中不友好的模块编号前缀：侧栏模块项仅显示教程名称，模块详情页抬头改为模块标签。
...
### 变更
- 本地调试默认屏蔽登录页：`src/App.jsx` 增加 `BYPASS_LOGIN` 与 `DEMO_USER`，未登录时以演示身份进入教程。
- 教程可见范围改为基于 `effectiveUser` 计算，确保演示身份可访问会员分层教程用于验收。

### 修复
- 修正“本地调试时卡在加载态/登录态”的体验问题，便于快速核对教程内容与交互。
...
- `INTERACTIVE_MAP.md`：记录全教程交互组件插入位置
...
### 新增
- CourseMap.jsx：Scrimba 风格教程地图主页（12 模块）
